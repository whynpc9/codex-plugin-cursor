...
function main() {
  ...
  const cwd = input.workspace_roots?.[0] || input.cwd || process.env.CURSOR_PROJECT_DIR || process.env.CLAUDE_PROJECT_DIR || process.cwd();
  ...
  if (!review.ok) {
    const message = runningTaskNote ? `${runningTaskNote} ${review.reason}` : review.reason;
    emitDecision({ followup_message: message });
    return;
  }
