...
- If the user is clearly giving a follow-up instruction such as "continue", "keep going", "resume", "apply the top fix", or "dig deeper", recommend continuing.
- Otherwise recommend starting a new thread.
...
- The subagent is a thin forwarder only. It should use one shell command to invoke `node "$CURSOR_PLUGIN_ROOT/scripts/codex-companion.mjs" task ...` and return that command's stdout as-is.
